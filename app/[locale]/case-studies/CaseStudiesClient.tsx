...
import { ArrowRight, MapPin, Calendar, Fish, Building2, CheckCircle, Clock, Award } from 'lucide-react';
...
export default function CaseStudiesClient({ projects, locale }: Props) {
        ...
        <div className="container-custom">
          ...
          <div className="text-center mb-12">
            <h2 className="text-4xl font-bold text-gray-900 mb-4">
              {locale === 'en' ? 'Featured Success Stories' : 'Рекомендовані історії успіху'}
            </h2>
            <p className="text-lg text-gray-600 max-w-3xl mx-auto">
              {locale === 'en'
                ? 'Explore detailed case studies showcasing our expertise across diverse aquaculture projects'
                : 'Дослідіть детальні кейси, що демонструють наш досвід у різних проєктах аквакультури'}
            </p>
          </div>
          ...
          {projects.length === 0 ? (
            <div className="text-center py-12 bg-white rounded-lg">
              <h3 className="text-2xl font-bold text-neutral-800 mb-4">
                {locale === 'en' ? 'No case studies available yet' : 'Кейси поки що недоступні'}
              </h3>
              <p className="text-neutral-600 mb-6">
                {locale === 'en'
                  ? 'Check back soon for detailed success stories from our completed projects.'
                  : 'Завітайте незабаром, щоб ознайомитися з детальними історіями успіху наших завершених проєктів.'}
              </p>
            </div>
          ) : (
                  ...
                  <Link key={project.id} href={`/${locale}/projects/${project.slug}`}>
                    <div className="border border-neutral-200 rounded-lg overflow-hidden hover:shadow-xl transition-all duration-300 bg-white h-full flex flex-col relative group">
                      ...
                      <div className="absolute top-3 left-3 z-10 bg-blue-600 text-white px-3 py-1 rounded-full text-xs font-semibold flex items-center gap-1">
                        ...
                        {locale === 'en' ? 'Case Study' : 'Кейс'}
                      </div>

                      {/* Featured Badge */}
                      {details?.projectFeatured && (
                        <div className="absolute top-3 right-3 z-10 bg-amber-500 text-white px-3 py-1 rounded-full text-xs font-semibold">
                      ...
                      <div className="relative h-48 w-full overflow-hidden bg-neutral-100">
                        {image?.sourceUrl ? (
                          ...
                          <div className="absolute inset-0 flex items-center justify-center bg-gradient-to-br from-blue-600/10 to-cyan-600/10">
                      ...
                      <div className="p-6 flex-1 flex flex-col">
                        ...
                        {/* Title */}
                        <h3 className="text-xl font-bold mb-2 text-neutral-900 group-hover:text-blue-600 transition-colors line-clamp-2">
                          {project.title}
                        </h3>
                        ...
                        {/* View Details Button */}
                        <button className="w-full mt-auto px-4 py-3 border-2 border-blue-600 text-blue-600 rounded-lg font-semibold hover:bg-blue-600 hover:text-white transition-all duration-300 flex items-center justify-center gap-2">
                          {locale === 'en' ? 'Read Full Case Study' : 'Читати повний кейс'}
